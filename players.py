...
def add_player(iss, name, role):
    ...
    try:
        sql = text("INSERT INTO players (iss, name, role, created_at) "\
                   "VALUES (:iss, :name, :role, NOW())")
...
def load_available_players_for_user(user_id):
    sql = text("SELECT p.id, p.iss, p.name, p.role FROM players p WHERE p.drafted=FALSE " \
               "AND NOT EXISTS( SELECT 1 FROM user_players up WHERE up.user_id=:user_id AND up.player_id=p.id) "\
               "ORDER BY iss ASC")
